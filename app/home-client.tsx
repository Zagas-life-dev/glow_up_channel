...
export default function Home() {
  ...
  // Signed-out visitors get the marketing landing page instead of the feed, so the
  // feed is only fetched once auth has resolved *and* there is someone to feed.
  const showLanding = !authLoading && !isAuthenticated

  // Use cursor pagination hook (stable fetchFunction like community page); session-restored feed when returning to page.
  // Defer first fetch until auth is ready so we call the correct endpoint and avoid double fetch.
  const {
    items: allContent,
    isLoading,
    isRefreshing,
  ...
  // True only while there is nothing on screen yet: auth is still resolving (the first
  // fetch is deliberately deferred until then), or the first page / a tab switch is in
  // flight. Paging in more items keeps the existing cards visible and uses the "loading
  // more" skeletons below the sentinel instead, so this stays false there.
  const feedLoading = authLoading || ((isLoading || isRefreshing) && allContent.length === 0)
  ...
  // Placed after every hook so the hook order stays identical on both branches.
    ...
    >
      ...
      >
        <div className="mx-auto flex max-w-2xl items-center gap-2">
          ...
          {/* Which country the feed is ranked for. The bar measures its own
              height into the spacer below, so this needs no layout changes. */}
          <CountrySelector align="end" className="shrink-0" />
        </div>
      </div>
