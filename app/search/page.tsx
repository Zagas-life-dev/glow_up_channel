    ...
    <PageShell fullWidth className="relative font-sans">
      <div className="mx-auto max-w-[760px]">
        ...
        <div className="min-w-0 pt-safe">
          <p className="text-xs font-bold uppercase tracking-[0.08em] text-up-orange-ink">
            Discover
          </p>
          <h1 className="mt-2 font-display text-[28px] font-bold leading-tight text-foreground sm:text-[34px]">
            Search
          </h1>
        ...
        <header className="sticky top-0 z-30 border-b border-border bg-up-bar pb-3 pt-2 backdrop-blur-xl">
          <div className="relative mt-4">
            ...
            <Input
              ...
              className="h-14 min-h-14 rounded-full pl-12 pr-12 text-base shadow-[0_1px_0_rgba(11,18,51,0.03)] placeholder:text-muted-foreground"
            ...
            {searchQuery ? (
              <button
                type="button"
                onClick={clearSearch}
                className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-up-fill hover:text-foreground"
          ...
          <nav
            className="scrollbar-hide -mx-1 mt-4 flex gap-1.5 overflow-x-auto px-1 pb-1"
            ...
            {TYPE_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "h-[38px] shrink-0 snap-start rounded-full px-[15px] text-sm font-semibold transition-colors",
                    active
        ...
        <main className="pb-8 pt-5 sm:pb-10 sm:pt-6">
          {error ? (
            <div className="mb-4 rounded-up-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-body-sm text-destructive">
              {error}
            </div>
          ) : null}

          {resourcesExcluded ? (
            <div className="rounded-up-xl border border-border bg-card px-6 py-14 text-center">
              <CalendarClock
            ...
            <ul className="space-y-3" aria-busy="true">
              {[...Array(5)].map((_, i) => (
                <li
                  key={i}
                  className="rounded-up-xl border border-border bg-card p-4 sm:p-5"
                >
                  <div className="animate-pulse space-y-4">
                    <div className="h-3.5 w-2/3 rounded-full bg-up-fill" />
            ...
            <div className="rounded-up-xl border border-dashed border-border bg-card px-6 py-16 text-center sm:py-20">
              <Sparkles
